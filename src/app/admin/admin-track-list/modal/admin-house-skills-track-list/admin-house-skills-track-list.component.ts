...
export class AdminHouseSkillsTrackListComponent implements OnInit {
  ...
  constructor(
    ...
    private skillService: SkillTrackService,
    public dialogRef: MatDialogRef<AdminHouseSkillsTrackListComponent>,
    ...
    if (data.type == "track") {
      ...
      this.houseService.getHousesByquize(this.data.housesid).subscribe((x) => {
        console.log('houses list popup');
        console.log(x);

        this.skills = x.class;
        console.log("skils list ");
  ...
  deleteSkill(skillid) {
    this.dialog
      .open(ConfirmDialogComponent, {
        data: {
          message: "Do you really want to delete the track from this class ?",
          title: "Delete Track"
        },
      })
      .afterClosed()
      .subscribe((ifYes) => {
        if (ifYes) {
          ...
          this.loading = true;
          this.skillService
            .deleteSkillByTrackId(this.data.trackid, skillid)
            .subscribe(
              (x: any) => {
  ...
  deleteHouse(houseid) {
    this.dialog
      .open(ConfirmDialogComponent, {
        data: {
          message: "Do you really want to delete the house from this class ?",
          title: "Delete House"
      ...
      .subscribe((ifYes) => {
        if (ifYes) {
          ...
          this.loading = true;
          this.houseService
            .deleteSkillByTrackId(this.data.housesid, houseid)
            .subscribe(
              (x: any) => {
                this.skills = x.skills;
                this.message = x.message;
