...
export class QuizService {
  ...
  public addQuiz(quiz) {
    return this.http.post<quiz>(environment.apiURL + '/quizzes', quiz).pipe(
  ...
  public allQuiz() {
    return this.http.get<quiz>(environment.apiURL + '/quizzes').pipe(
      map((response) => response),
      catchError((error: any) => throwError(error || { message: 'Server Error' })),);
  }

  public getQuiz(id) {
    return this.http.get(`${environment.apiURL}/quizzes/${id}`).pipe(
      map((response) => response),
      catchError((error: any) => throwError(error || { message: 'Server Error' })),);
  }
  ...
  deleteQuizWithAll(id: String): Observable<quiz[]> {
    ...
    return this.http.delete<quiz[]>(url).pipe(
      map((response) => response),
      catchError((error: any) => throwError(error || { message: 'Server Error' })),);
  }
